...
echo use_javascript(plugin_web_path('esimple_draggtable', 'jquery.dragtable.js'));
echo use_stylesheet(plugin_web_path('esimple_draggtable', 'dragtable.css'));

echo use_javascript(plugin_web_path('extras', 'jquery.tablednd.js'));
...
<script>
    ...
    function getPrint(){
        ...
        tableToExcel('resultTable',  'Rapport', 'Rapport'+new Date()+'.xls');
    ...
    $(document).ready(function(){
        ...
        $('#resultTable').dragtable({maxMovingRows:2});
        jQuery('#resultTable').tableDnD();
    });
...
<div style="float:right;">
    ...
    <table>
        <tr>
            <td><input type="button" value="Print"  class="btn" onclick="getPrint()"   /></td>
...
if ($hasSelectableRows) {
    ...
    $headerRow1 .= content_tag('th', $selectAllCheckbox->__toString(),
            array('rowspan' => $selectAllRowspan, 'class' => 'checkbox-col' ,'id'=>'foo_baz')) . '<label for="ohrmList_chkSelectAll">Sélectionner Tout</label>'."\n";
...
foreach ($headerGroups as $group) {
    ...
    foreach ($group->getHeaders() as $header) {
        ...
        $headerCellHtml = '<th  class="dragtable-drag-boundary" rowspan="' . $rowspan . '" style="width:' . $header->getWidth() . '"' . $headerCellClassHtml . '>' . $headerCell->__toString() . "</th>\n";
